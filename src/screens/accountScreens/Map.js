  ...
  useEffect(() => {
    const getAddressFromCoords = async () => {
      if (currentPosition) {
        try {
          const response = await fetch(
            `https://api.geoapify.com/v1/geocode/reverse?lat=${currentPosition.latitude}&lon=${currentPosition.longitude}&apiKey=${GEOAPIFY_API_KEY}&lang=vi`
          );
          const data = await response.json();
        ...
        <Button
          ...
          onPress={() => {
            if (currentPosition && address) {
              ...
              navigation.navigate(previousScreen || 'DetailAccountScreen', { 
